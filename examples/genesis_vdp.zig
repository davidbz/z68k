...
//!
//! Knows nothing about the 68000 or the bus: the machine in `genesis.zig`
//! forwards port accesses here and runs the one DMA mode that needs to read
//! the 68k bus. Rendering is per scanline, which is what line-scroll effects
//! (Sonic's water, the title screen's wobble) need and no more.
//!
...
//! each is a self-contained addition to the pixel loop below.
